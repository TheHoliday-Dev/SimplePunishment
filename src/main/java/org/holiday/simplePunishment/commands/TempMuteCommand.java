...
public class TempMuteCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("punishment.tempmute")) {
            sender.sendMessage(ChatColor.RED + "You do not have permission to use this command.");
            return true;
        }
        if (args.length < 3) {
            sender.sendMessage(ChatColor.RED + "Usage: /tempmute <player> <seconds> <reason>");
            return true;
        }
        ...
        String tempMuteMsg = Main.getInstance().getConfig().getString("messages.tempmute")
                .replace("%reason%", reason)
                .replace("%expiration%", expiration.toString());
        sender.sendMessage(ChatColor.GREEN + "Player " + target.getName() + " has been temporarily muted. " + tempMuteMsg);
        return true;
    }
}
